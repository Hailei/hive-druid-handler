...
public class DruidRecordWriter implements RecordWriter<NullWritable, DruidWritable>,
  ...
  private final MetaStorageHolder holder;
  
  private final Supplier<Committer> committerSupplier;
  
  private final List<DataSegment> pushedSegment;
  ...
  public DruidRecordWriter(
    ...
    pushedSegment = new ArrayList<DataSegment>();
    this.holder = holder;
   ...
   */
  private SegmentIdentifier getSegmentIdentifierAndMaybePush(long timestamp,int shard) {

    final Granularity segmentGranularity = dataSchema.getGranularitySpec()
            .getSegmentGranularity();
   DateTime eventTime = new DateTime(timestamp);
    final Interval interval = new Interval(
            segmentGranularity.bucketStart(eventTime),
            segmentGranularity.bucketEnd(eventTime)
    );

    SegmentIdentifier retVal = currentOpenSegment;
    if (currentOpenSegment == null) {
      retVal = new SegmentIdentifier(
              dataSchema.getDataSource(),
              interval,
              tuningConfig.getVersioningPolicy().getVersion(interval),
              new LinearShardSpec(shard)
      );
  ...
  private void pushSegments(List<SegmentIdentifier> segmentsToPush) {
    try {
      ...
      pushedSegment.addAll(segmentsAndMetadata.getSegments());
  ...
  public void write(Writable w) throws IOException {
    ...
    final long timestamp = (long) record.getValue().get(Constants.DEFAULT_TIMESTAMP_COLUMN);
    final long truncatedTime = (long) record.getValue()
           .get(partitionCols.get(0));
    final int shard = (int)record.getValue().get(partitionCols.get(1));
    ...
    try {
      appenderator
              .add(getSegmentIdentifierAndMaybePush(truncatedTime,shard), inputRow, committerSupplier);
  ...
  public void close(boolean abort) throws IOException {
    try {
      ...
      if(abort == false){
    	  ...
    	  holder.druidMetaStorageUpdate(pushedSegment);
      }
    } catch (InterruptedException e) {
      Throwables.propagate(e);
    } finally {
