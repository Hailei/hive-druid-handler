...
public class DruidQueryBasedInputFormat extends InputFormat<NullWritable, DruidWritable>
  ...
  private HiveDruidSplit[] getInputSplits(Configuration conf) throws IOException {
    String address = conf.get(Constants.HIVE_DRUID_BROKER_DEFAULT_ADDRESS,"localhost:8082");
    ...
    switch (druidQueryType) {
      ...
      case Query.SELECT:
        ...
        boolean distributed = conf.getBoolean(Constants.HIVE_DRUID_SELECT_DISTRIBUTE, true);
        if (distributed) {
  ...
  private static HiveDruidSplit[] distributeSelectQuery(Configuration conf, String address,
    ...
    final int numConnection = conf.getInt(Constants.HIVE_DRUID_NUM_HTTP_CONNECTION, 25);
    final Period readTimeout = new Period(
           conf.get(Constants.HIVE_DRUID_HTTP_READ_TIMEOUT,"PT1M"));
  ...
  private static HiveDruidSplit[] splitSelectQuery(Configuration conf, String address,
    ...
    final int selectThreshold = conf.getInt(Constants.HIVE_DRUID_SELECT_THRESHOLD, 10000);
    final int numConnection = conf.getInt(Constants.HIVE_DRUID_NUM_HTTP_CONNECTION, 25);
    final Period readTimeout = new Period(
            conf.get(Constants.HIVE_DRUID_HTTP_READ_TIMEOUT,"PT1M"));
